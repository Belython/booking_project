...
public class AlterHotelsCommand extends AbstractCommand {
    ...
    public ServletAction execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            switch (subCommand) {
                case Value.ADD_NEW: {
                    HotelServiceImpl.getInstance().addList(hotelDtoList);
                    break;
                }
                case Value.CHANGE_EXISTING: {
                    HotelServiceImpl.getInstance().updateList(hotelDtoList);
                    break;
                }
            }
            List<HotelDto> newHotelDtoList = HotelServiceImpl.getInstance().getAll();
