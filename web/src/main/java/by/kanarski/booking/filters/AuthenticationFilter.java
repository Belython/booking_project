...
//    private List<String> adminURL = new PathList().setListURLAdmin();
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        UserDto user = (Client) httpRequest.getSession().getAttribute(CLIENT);
        ...
        } else if (user != null & requestURI.equals(httpRequest.getContextPath() + Pages.PAGE_USER)) {
            getRole(request, response, user);
            //check authorization
        } else if (user != null) {
            //if user has role user, only user URLs
            if (user.getRole().getName().equals(USER) & requestURI.equals(checkIn(httpRequest, adminURL))) {
