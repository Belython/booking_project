					...
					<article>
						...
						<form id="bookRoomsForm" action="bookRooms" method="POST">
							...
							<c:forEach var="roomType" items="${roomTypeSet}">
								...
								<c:set var="availableRooms" value="${roomTypesCount.get(roomType)}"/>
								<c:set var="pricePerNight" value="${loc:getMoney(roomType.pricePerNight)}"/>
								<!--room-->
								<li>
												...
												<select name="orderedRooms[${r}].amount">
													<c:forEach begin="0" end="${availableRooms}" var="i">
														<c:choose>
															...
															<c:otherwise>
																<option value="${i}">${i} $ (${i * pricePerNight})</option>
															</c:otherwise>
														</c:choose>
													</c:forEach>
												</select>
							...
							<input type="submit" id="book" value="<spring:message code="selectRoom.submitRooms"/>" class="gradient-button"/>
						</form>
